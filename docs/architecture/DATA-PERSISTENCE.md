...
QA/retention evidence uses its own `budget-cart:qa:*` keys in the same browser storage (timing QA in tab-scoped `sessionStorage`) and is never canonical shopping state. The appearance and scan-mode preferences are convenience state outside these rules.
